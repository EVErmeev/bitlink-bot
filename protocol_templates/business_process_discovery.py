...
class BusinessProcessDiscoveryTemplate(BaseProtocolTemplate):
    ...
    def assemble_from_llm_json(self, protocol: Protocol, atomic_items: list,
        ...
        protocol.meeting_purpose = llm_data.get("purpose", llm_data.get("meeting_purpose", ""))
        protocol.processes_as_is = llm_data.get("processes_as_is", "")
        protocol.roles_description = llm_data.get("roles_description", "")
        protocol.systems_and_documents = llm_data.get("systems_and_documents", "")
        protocol.process_steps = llm_data.get("process_steps", [])
        protocol.problems = llm_data.get("problems", [])
        protocol.requirements = llm_data.get("requirements", [])
        protocol.integration_points = llm_data.get("integration_points", "")
